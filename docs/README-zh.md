...
### 原则: 在用户动态授权后, 由相互不可见的self私钥和web2服务私钥各自签名担保，再由web3合约进行强制性验证后进行链上操作， 包括密文数据查询和私有web3数据更新等, 三方相互制约, 保证去中心化运作的同时, 提供高度安全的隐私保护

![/docs/selfweb3-arch.png](/docs/selfweb3-arch.png)

#### 说明: 公私钥指椭圆曲线secp256k1算法生成的公私钥对，密钥指32位明文字符串，dhKey指通过私钥和公钥动态计算出来的共享密钥

> - web3合约: 作为密钥防篡改节点，一经注册写入，无法更改，链上数据获取和更新依赖于用户私有动态授权以及链上强制性的关联验证
...
> - 关联验证: 作为证明机制保证私有动态授权的有效性, 包括链下多方签名动态生成，以及默克尔树证明生成

> - 链上验证: 作为证明机制保证链上操作的合法性, 包括链上多方签名验证和默克尔树关联性和重放攻击验证
...
> - web2Nonce: 作为默克尔树证明数据源为关联验证提供支持
> - selfData: 为用户提供私有数据密文备份存储服务, 提升用户体验
> - web2私钥: web2服务自身的私钥, 用于生成web2签名以及动态合成web2DHKey
> - web3公钥: 用于生成私有地址以及动态合成web2DHKey和selfDHKey, web3私钥丢弃
> - web2DHKey: 由web2私钥和web3公钥动态计算生成的共享密钥, 用于加解密self私钥

### web3合约存储: 私有密钥 + web2密钥混合加密后存储
> - web2地址: 用于验证web2服务签名
> - 恢复ID: web3私钥签名 + 钱包签名验证
> - selfKey: selfDHKey加密 + self私钥签名验证
> - self私钥: web2DHKey加密 + selfPass加密 + 钱包签名验证
   
### 私有数据存储: selfPass加密后存储 => web2 + web3
> - selfNonce: 动态生成用于私有签名
> - TOTP密钥: 用于TOTP验证, selfDHKey加密
> - web3公钥: 用于生成私有地址以及动态合成selfDHKey
> - selfPass: 用户输入或动态生成, 用于解密self私钥，经钱包加密存储
> - web2DHKey: 由web2私钥和web3公钥动态计算生成的共享密钥, 用于加解密self私钥

## 特性

### 关联验证(链下默克尔树证明生成 + 链下签名生成 + 链上动态验证):

1. 实现目的: 强制性在链上验证关联动态授权的有效性, 只有所有签名验证通过才能在链上完成敏感操作, 同时通过默克尔树验证叶子结点有效性防止重放攻击
2. 实现原理: web2服务和wasm部分别生成动态随机数, 加入已有的默克尔树后生成相应证明, 同时两部分分别使用各自的私钥对随机数进行签名, 所有参数打包后送进合约进行签名验证和随机数有效性验证, 关联验证路径在合约中针对具体业务硬编码
3. 实现方式: web2服务针对链上动态验证进行默克尔树管理, 并动态生成签名进行担保, wasm部分通过self私钥生成签名和随机数保证私有操作, web3合约负责验证所有签名和默克尔树证明的有效性
...
1. 作为隐私计算节点，处理私有密钥和私有数据相关的所有明文计算任务, 以及私有动态授权的链下校验，同时所有操作依赖于支持自行重置的密钥selfPass，用于支持用户数据隐私安全

2. wasm文件合法性由wasm文件嵌入的动态验证库进行支持, 包括对文件hash值进行签名验证等

3. webAuthn数据由wasm部分采用随机密钥加密存储，随机密钥解密依赖于链上的selfKey，该密钥提取需要通过TOTP动态授权以及关联验证
...
> - WebAuthn: 在成功获取到selfKey后解密WehAuthn凭证密文, 参与关联验证, 由链上验证动态授权有效性
...
> - 流程说明: 如果开启web3模式, 由于没有web2服务的支持, 无法获取到动态签名, 为了保证链上敏感操作的高度安全, web3合约会针对链上更新的操作强制性验证两部分的签名以及默克尔树叶子节点的有效性, 之所引入web2服务的签名验证, 是为了通过web2服务和用户自己私有的签名加强关联验证的安全性, 同时只有wasm部分才能获取到用户私有密钥和数据, 通过生成动态随机数并签名的形式防止关联验证的非法通过
...
3. selfPass是由用户自行输入或者wasm部分动态生成的随机密钥，同时支持通过钱包加密存储到web3去中心化平台

4. web3公钥由wasm部分随机生成, 并在对恢复ID签名后丢弃私钥, 用于生成用户唯一的私有地址以及参与共享密钥的计算, 经加密后存储到web2服务和web3去中心化存储平台

4. self私钥由web2DHKey和selfPass关联加密后存储在web3合约中, 用于与web3公钥动态计算合成selfDHKey, 一经注册写入合约无法更改

5. selfKey由selfDHKey加密后存储在web3合约中，用于用户数据加密后存储到web2服务和web3去中心化存储平台，比如webAuthn凭证

6. 重置功能包括TOTP, WebAuthn, selfPass以及用户钱包，所有重置操作都会启用完整的关联验证流程用于加强安全防护, 同时TOTP和WebAuthn重置需要其中的一个有效, 否则需要通过除Email校验外的的审核验证
