...
export default {
                            ...
                            btns.push(h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                            ...
                                            if (self.items.data[params.index]['data']['url'] === undefined) self.showPanel(self.items.data[params.index]['key'], self.items.data[params.index]['data']['value']);
    ...
    methods: {
            ...
            // self.$parent.getSelf().httpPost('/api/user/recover', formdata, function(response){
                if (response.data['Error'] !== '' && response.data['Error'] !== null && response.data['Error'] !== undefined) {
                    ...
                    var templateParams = {
                        ...
                        message: "[selfweb3] code for dynamic authorization: " + response.data['Data']
                    };
        ...
        showEnDecrypt() {
            ...
            this.$parent.getSelf().afterVerify(true, 'cryptoPanel', null);
        },
        showPanel(name, params) {
            this.$parent.getSelf().afterVerify(true, name, params);
            // this.$parent.getSelf().switchPanel('verify', name, params);
        },
        showQRcode(totpKey) {
            ...
            const totpName = 'selfweb3-' + this.$parent.getSelf().getWallet().network + ':' + walletAddr;
