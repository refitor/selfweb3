...
contract selfweb3 is Ownable {
     ...
     */
    function Register(bytes calldata selfID, bytes calldata recoverID, bytes calldata web3Key, bytes calldata web3Public) external payable {
     ...
     * @param message random string dynamically generated by the front end.
     */
    function Rebind(bytes calldata selfID, address wallet, bytes calldata signature, bytes calldata message) external payable {
        SelfData memory sd = _getKV(selfID);
     ...
     * @param selfID unique user ID.
     */
    function Web3Key(bytes calldata selfID) view external returns (bytes memory web3Key) {
        SelfData memory sd = _getKV(selfID);
        require(sd.web3Public.length != 0, "not registered yet");
        return sd.web3Key;
    }
