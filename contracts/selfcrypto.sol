...
contract selfweb3 is Ownable {
     ...
     */
    function Register(bytes calldata userID, bytes calldata recoverID, bytes calldata web3Key, bytes calldata web3Public) external payable {
        ...
        sd = SelfData(recoverID, web3Key, web3Public, msg.sender);
        _setKV(userID, sd);
    }
     ...
     * @param message random string dynamically generated by the front end.
     */
    function Rebind(bytes calldata userID, address wallet, bytes calldata signature, bytes calldata message) external payable {
        SelfData memory sd = _getKV(userID);
        ...
        require(sd.web3Public.length != 0, "not registered yet");
        sd = SelfData(sd.recoverID, sd.web3Key, sd.web3Public, msg.sender);
        _setKV(userID, sd);
    }
     ...
     * @param userID unique user ID.
     */
    function Web3Key(bytes calldata userID) view external returns (bytes memory web3Key) {
        SelfData memory sd = _getKV(userID);
        require(sd.web3Public.length != 0, "not registered yet");
        return sd.web3Key;
    }
