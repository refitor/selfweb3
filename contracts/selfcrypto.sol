...
contract selfweb3 is Ownable {
     ...
     */
    function Load(bytes calldata signature, bytes calldata message) view external returns (bytes memory recoverID, bytes memory web3Public) {
     ...
     * @dev Load is used to load the key information.
     */
    function Web3Key() view external returns (bytes memory web3Key) {
        SelfData memory sd = _getKV(msg.sender);
        require(sd.web3Public.length != 0, "not registered yet");
        return sd.web3Key;
    }
